...
let COUNT = {
	"PollenParticleSystem" : 25,
	"BeeParticleSystem" : 4,
	"ButterflyParticleSystem" : 2
}

let DEBUG_DRAW = {
	"PollenParticleSystem": false,
	"BeeParticleSystem" : false,
	"ButterflyParticleSystem" : false
}
...
let sliderNames = {
	...
	"pollenDrag" : .5,
	"beeThrust": .5,
	"butterflyBorderForce" : .5,
	"pollenSeparation" : .5,
	"beeTurnSpeed" : .5,
	"butterflyMouseAttraction" : .5
}
...
function createSliders() {
	let numHolder = document.getElementById("counts")
	systemClasses.forEach(sys => {
		...
		console.log(name)
 		...
 		let val = COUNT[name]
 		let numBox = Object.assign(document.createElement("input"),{
			type:"number",
			min: "0",
			id:"number" + name,
			value: val
		})
		label.innerText = name.replace("ParticleSystem", "") + " Count"
		numHolder.append(div)
		div.append(label)
		div.append(numBox)
		let fxn = `COUNT.${name} = this.value;`
		numBox.setAttribute("oninput", fxn);
	})
	...
	systemClasses.forEach(sys => {
		...
		label.innerText = name.replace("ParticleSystem", "") + " Debug View"
	...
	keys.forEach(name => {
		...
		div.classList.add('slider')
		label.classList.add('label')
		label.innerText = name.charAt(0).toUpperCase() + name.substring(1).split(/(?=[A-Z])/).join(" ")
